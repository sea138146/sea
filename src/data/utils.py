...
def load_hf_dataset(path, **kwargs):
    """Load HuggingFace dataset, with local fallback for TOFU."""

    from pathlib import Path
    import datasets

    if path == "locuslab/TOFU":
        tofu_local_dir = Path("/data/datasets/datasets-TOFU")
        tofu_splits = {
        ...
        if name is None and isinstance(split, str) and split in tofu_splits:
            name = split

        if name is None:
            raise ValueError(
                "Local TOFU loading requires a split name such as forget05, "
                "retain95, or holdout05."
            )

        local_file = tofu_local_dir / f"{name}.json"
        if not local_file.exists():
            raise FileNotFoundError(f"Local TOFU file not found: {local_file}")
