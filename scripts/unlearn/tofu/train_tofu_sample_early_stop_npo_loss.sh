...
TRAINER="SampleEarlyStopNPOLoss"
    ...
    for lr in "${lr_set[@]}"; do
        for bz in "${bz_set[@]}"; do
            ...
            for epochs in "${epoch_set[@]}"; do
                SUFFIX="lr${lr}_b${bsz}_ga${grad_acc}_e${epochs}_ies_npoloss_tau${threshold}_stable2_rebound2_x2"
                ...
                python src/train.py --config-name=unlearn.yaml \
                    ...
                    trainer.method_args.rebound_multiplier=2.0 \
                    trainer.method_args.rebound_checks=2 \
                    trainer.method_args.calibration_only=false
                # ============================================================
