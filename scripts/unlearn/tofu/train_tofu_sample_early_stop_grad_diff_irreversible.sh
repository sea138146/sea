...
for split in "${splits[@]}"; do
    ...
    test -f "${retain_logs}" || {
        ...
        echo
        echo "Available matching retain files:"
        find "${EVAL_ROOT}" -type f \
            -path "*Llama-3.2-1B-Instruct*${retain_split}*/TOFU_EVAL.json" \
        ...
        for lr in "${lr_set[@]}"; do
            for bz in "${bz_set[@]}"; do
                ...
                for epochs in "${epoch_set[@]}"; do
                    suffix="lr${lr}_b${bsz}_ga${grad_acc}_e${epochs}_ies_graddiff_irreversible_d2absmean3_tau${threshold}"
