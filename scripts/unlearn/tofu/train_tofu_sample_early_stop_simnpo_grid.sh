...
for split in "${splits[@]}"; do
    ...
    for bz in "${bz_set[@]}"; do
        ...
        for epochs in "${epoch_set[@]}"; do
            SUFFIX="lr${lr}_b${bsz}_ga${grad_acc}_beta${beta}_delta${delta}_gamma${gamma}_e${epochs}_marginalratio_ma${moving_average_window}_stop${stop_ratio_threshold}_rebound${rebound_ratio_threshold}_samplingbaseline_masked"
            ...
            echo
            echo "============================================================"
            echo "开始 SimNPO marginal-ratio 早停训练：${TASK_NAME}"
