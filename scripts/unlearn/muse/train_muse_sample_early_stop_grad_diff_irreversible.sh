...
INITIAL_NLL_CACHE_ROOT="${CHECKPOINT_ROOT}/initial_nll/${MODEL_TAG}/NPO"
...
alpha_set=("1") # standard GradDiff retain weight
warm_up_set=("0")
patience_set=("1")
rebound_delta_set=("0.05")
...
for SPLIT_CONFIG in "${splits[@]}"; do
    read -r DATA_SPLIT THRESHOLD_A THRESHOLD_B <<< "${SPLIT_CONFIG}"
...
SUFFIX="lr${LR}_b${BATCH_SIZE}_ga${GRAD_ACC}_alpha${ALPHA}_e${EPOCHS}_ies_graddiff_normnll_gain_ge${GAIN_THRESHOLD}_warm${WARM_UP}_patience${PATIENCE}_rebounddelta${REBOUND_DELTA}_rpat${REACTIVATION_PATIENCE}_samplingbaseline_masked"
...
echo "============================================================" | tee -a "${LOG_FILE}"
echo "MUSE ${DATA_SPLIT} GradDiff sample early stop with rebound" | tee -a "${LOG_FILE}"
