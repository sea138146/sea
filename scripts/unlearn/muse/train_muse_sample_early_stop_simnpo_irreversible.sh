...
INITIAL_NLL_CACHE_ROOT="${CHECKPOINT_ROOT}/initial_nll/${MODEL_TAG}/SimNPO"
...
bz_set=("8 4") # batch_size, gradient_accumulation_steps
alpha_set=("1") # SimNPO retain weight
beta_set=("4.5")
delta_set=("0.0")
gamma_set=("0.125")
warm_up_set=("2")
...
for SPLIT_CONFIG in "${splits[@]}"; do
    read -r DATA_SPLIT SPLIT_LR THRESHOLD_A THRESHOLD_B <<< "${SPLIT_CONFIG}"
...
for GAIN_THRESHOLD in "${THRESHOLD_A}" "${THRESHOLD_B}"; do
...
SUFFIX="lr${LR}_b${BATCH_SIZE}_ga${GRAD_ACC}_beta${BETA}_delta${DELTA}_gamma${GAMMA}_alpha${ALPHA}_e${EPOCHS}_ies_simnpo_normnll_gain_ge${GAIN_THRESHOLD}_warm${WARM_UP}_patience${PATIENCE}_rebounddelta${REBOUND_DELTA}_rpat${REACTIVATION_PATIENCE}_samplingbaseline_masked"
...
echo "============================================================" | tee -a "${LOG_FILE}"
echo "MUSE ${DATA_SPLIT} SimNPO sample early stop with rebound" | tee -a "${LOG_FILE}"
...
echo "epochs=${EPOCHS} alpha=${ALPHA} beta=${BETA} delta=${DELTA} gamma=${GAMMA}" | tee -a "${LOG_FILE}"
echo "warm_up=${WARM_UP} patience=${PATIENCE} gain_threshold=${GAIN_THRESHOLD}" | tee -a "${LOG_FILE}"
echo "rebound_delta=${REBOUND_DELTA} rebound_threshold=${GAIN_THRESHOLD}-${REBOUND_DELTA}" | tee -a "${LOG_FILE}"
echo "reactivation_patience=${REACTIVATION_PATIENCE}" | tee -a "${LOG_FILE}"
echo "initial_nll_cache=${INITIAL_NLL_CACHE}" | tee -a "${LOG_FILE}"
echo "sampling=baseline_forget_anchor_with_masked_forget_loss" | tee -a "${LOG_FILE}"
echo "output_dir=${OUTPUT_DIR}" | tee -a "${LOG_FILE}"
echo "training_eval=disabled final_eval=enabled" | tee -a "${LOG_FILE}"
echo "============================================================" | tee -a "${LOG_FILE}"

# Stopped forget chunks keep their baseline batch slots but contribute zero
# forget loss. Retain loss remains active at every step; all forget chunks are
# monitored forward-only at every epoch end for stopping and reactivation.
