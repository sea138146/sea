...
SUFFIX="lr${LR}_b${BATCH_SIZE}_ga${GRAD_ACC}_beta${BETA}_delta${DELTA}_gamma${GAMMA}_alpha${ALPHA}_e${EPOCHS}_nlltrace"
...
echo "============================================================" | tee -a "${LOG_FILE}"
echo "MUSE ${DATA_SPLIT} SimNPO baseline" | tee -a "${LOG_FILE}"
...
echo "beta=${BETA} delta=${DELTA} gamma=${GAMMA}" | tee -a "${LOG_FILE}"
...
# Training does not run the expensive MUSE evaluator at every epoch. The
# SimNPO trainer still writes one forward-only normalized-NLL snapshot for every
