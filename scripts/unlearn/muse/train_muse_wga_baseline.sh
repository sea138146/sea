...
alpha_set=("1") # standard WGA retain weight
beta_set=("1.0") # WGA default; add "5.0" here for an extended grid
epoch_set=("10")
...
SUFFIX="lr${LR}_b${BATCH_SIZE}_ga${GRAD_ACC}_beta${BETA}_alpha${ALPHA}_e${EPOCHS}_nlltrace"
...
echo "============================================================" | tee -a "${LOG_FILE}"
echo "MUSE ${DATA_SPLIT} WGA baseline" | tee -a "${LOG_FILE}"
...
# Training does not run the expensive MUSE evaluator at every epoch. The WGA
